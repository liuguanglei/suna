...
export function WebSearchToolView({
  ...
  const getResultType = (result: any) => {
    ...
    if (
      ...
      return { icon: FileText, label: '文章' };
    } else if (url.includes('wiki')) {
      return { icon: BookOpen, label: '维基' };
    } else if (url.includes('blog')) {
      return { icon: CalendarDays, label: '博客' };
    } else {
      return { icon: Globe, label: '网站' };
    }
  };
        ...
        <div className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="relative p-2 rounded-lg bg-gradient-to-br from-neutral-200 to-neutral-300 border border-neutral-200">
              <Search className="w-5 h-5 text-neutral-600 dark:text-neutral-600" />
            </div>
            <div>
