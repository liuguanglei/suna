...
function SafeImage({
  ...
  const handleError = () => {
    if (attempts < 3) {
      ...
      console.log(
        `Image load failed (attempt ${attempts + 1}). Trying alternative:`,
        filePath,
      );
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-64 bg-gradient-to-b from-rose-50 to-rose-100 dark:from-rose-950/30 dark:to-rose-900/20 rounded-lg border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 shadow-inner">
        ...
        <p className="text-sm font-medium">无法加载图片</p>
...
export function SeeImageToolView({
  ...
  const config = {
    color: 'text-neutral-600 dark:text-neutral-600',
    bgColor: 'bg-gradient-to-b from-neutral-200 to-neutral-300',
    badgeColor:
      'bg-gradient-to-b from-neutral-200 to-neutral-100 text-neutral-700 shadow-sm dark:from-neutral-800/50 dark:to-neutral-900/60 dark:text-neutral-300',
      ...
      <CardHeader className="h-14 bg-gradient-to-r from-zinc-50/90 to-zinc-100/90 dark:from-zinc-900/90 dark:to-zinc-800/90 backdrop-blur-sm border-b p-2 px-4 space-y-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div
              className={cn(
                'relative p-2 rounded-lg bg-gradient-to-br from-blue-500/20 to-blue-600/10 border border-blue-500/20 transition-colors',
                config.bgColor,
              )}
            >
