...
export const ChatInput = forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const { data: subscriptionData } =
      useSubscriptionWithStreaming(isAgentRunning);
    ...
    useEffect(() => {
      if (
        ...
        onAgentSelect &&
        !hasLoadedFromLocalStorage.current &&
        agents.length > 0
      ) {
        ...
        if (!selectedAgentId && !hasAgentIdInUrl) {
          ...
          if (savedAgentId) {
            ...
            if (defaultSunaAgent) {
              console.log(
                'Auto-selecting default Suna agent:',
                defaultSunaAgent.agent_id,
              );
              onAgentSelect(defaultSunaAgent.agent_id);
            } else if (agents.length > 0) {
              console.log(
                'No default Suna agent found, selecting first available agent:',
                agents[0].agent_id,
              );
    ...
    useEffect(() => {
      if (typeof window !== 'undefined' && agents.length > 0) {
        ...
        console.log(
          'Saving selected agent to localStorage:',
          keyToStore,
          'for selectedAgentId:',
          selectedAgentId,
        );
        ...
        <div className="relative">
          ...
          >
            <div className="w-full text-sm flex flex-col justify-between items-start rounded-lg">
              <CardContent
                className={`w-full p-1.5 ${enableAdvancedConfig && selectedAgentId ? 'pb-1' : 'pb-2'} ${bgColor} border ${enableAdvancedConfig && selectedAgentId ? 'rounded-t-3xl' : 'rounded-3xl'}`}
              >
                <AttachmentGroup
                  files={uploadedFiles || []}
                  sandboxId={sandboxId}
